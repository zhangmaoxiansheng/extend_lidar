...
class Trainer:
    def __init__(self, options):
        ...
        if self.use_pose_net:
            if self.opt.pose_model_type == "separate_resnet":
                ...
                self.models["pose"] = networks.PoseDecoder2(
        ...
        self.models["pose_encoder"] = nn.DataParallel(self.models["pose_encoder"])
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            last_epoch = batch_idx % self.opt.log_frequency == 0 and self.epoch == self.opt.num_epochs - 1
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        for j in range(min(4, self.opt.batch_size)):  # write a maxmimum of four images
            # if self.refine:
            #     writer.add_image("ref_disp",normalize_image(outputs[("disp",'r')][j]), self.step)
