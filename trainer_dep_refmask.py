...
class Trainer:
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in scales:
            ...
            depth_part_gt[depth_part_gt == 0] = 1
            ...
            # if scale == 'r':
            #     depth_ssim_loss = self.ssim(disp, outputs[("disp",scale)]*outputs['scale']).mean()
            #     depth_loss = depth_ssim_loss * 0
            ...
            #losses["smooth/{}".format(scale)] = smooth_loss

            # if scale == 'r':
            #     mask = inputs["depth_gt_part"].sign()
            #     norm_disp_part = norm_disp * mask
            #     smooth_loss_r = get_smooth_loss(norm_disp_part, color*mask)
            #     losses["smooth/r_{}".format(scale)] = smooth_loss_r
                

            # if scale == 'r':
            #     loss += self.opt.disparity_smoothness * smooth_loss
            #     loss += self.opt.disparity_smoothness * smooth_loss_r * 50
